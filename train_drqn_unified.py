#!/usr/bin/env python3
"""DRQN 实验2: 统一决策(同时决定) + γ=0 + 纯Cavai(N+1) + 纯rate reward"""
...
H, SEQ, BATCH, TU, GAMMA = 128, 6, 256, 10, 0.0  # γ=0: 不bootstrap
...
def test_20eps(qnet, label):
    ...
    for ep in range(5):
        ...
        decisions = {}
        for a in agents:
            decisions[a.user] = a.decide(env_t.Observe(a.user, 'NETWORK_LOAD')) + 1
        env_t.step(decisions, 'NETWORK')
    ...
    for ep in range(20):
        ...
        for a in agents:
            ...
            decisions[a.user] = a.decide(obs) + 1
        env_t.step(decisions, 'NETWORK')
        for a in agents:
            ...
            if feeder and a.user.sat_connected:
                ...
                act_id = a.user.sat_connected.ID - 1
                chosen_rank = next((r for r, (sid, _) in enumerate(visible_fb) if sid == act_id), len(visible_fb)-1)
...
print('DRQN Exp2: Unified-decision + gamma=0 + Cavai-only(N+1) ISL=2000 200u 200eps', flush=True)
...
for ep in range(200):
    ...
    # === 统一决策: 所有用户先observe+decide (同一状态) ===
    stored = []
    for a in agents:
        obs = env.Observe(a.user, 'NETWORK_LOAD')
        act = a.decide(obs)
        stored.append((a, obs, act))

    # === 一次性应用所有决策 ===
    decisions = {a.user: act+1 for a, obs, act in stored}
    env.step(decisions, mode)

    # === 统一计算reward ===
    for a, obs, act in stored:
    ...
    if ep >= SEQ+1:
        ...
        if len(s_l) > 0:
            ...
            with torch.no_grad():
                ...
                target = r_last + GAMMA * nq.max(dim=1).values  # γ=0 → target=r
    ...
    if ep > 0 and ep % 50 == 0:
        torch.save(qnet.state_dict(), f'./log/model/drqn_unified_ep{ep}.pkl')

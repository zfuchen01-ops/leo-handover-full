#!/usr/bin/env python3
"""Transformer 版 replay: 与 l0_replay.py 同口径(immediate stat + time_step=50),
唯一区别是构造 UserAgent 前设 LEO_USE_TRANSFORMER=1 + LEO_FEAT_PER_SAT=4,
让 UserAgent.step 走 transformer 分支, 并加载 TransformerQ_net 权重。

用法:
  python3 l0_replay_tf.py --model log/model/drqn_A_u100_tf_smoke_ep200.pkl \
      --const A --users 100 --eps 100 --freeze-gaps ''
"""
...
# ★ Transformer 专用: 必须在 import train_drqn / DRQNAgent 之前设
os.environ['LEO_USE_TRANSFORMER'] = '1'
os.environ['LEO_FEAT_PER_SAT'] = '4'
os.environ.setdefault('LEO_VARLEN', '0')
sys.path.insert(0, '.')
...
print(f"=== L0-b replay (TRANSFORMER): {args.const}x{args.users} eps={args.eps} ===", flush=True)

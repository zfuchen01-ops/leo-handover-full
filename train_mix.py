...
for cy in range(CYCLES):
 for d in DENSITIES:
  User.uid=0;Topology.index_con=0
  env=build_drqn_env(d,CONST);c.env=env
  ua=[]
  for i in range(d):
   a=UserAgent(env.topo.user[i],env,c,gamma=0.9,epsilon=0.1,batch=256,buffer=2000,hidden_size=128,seq=6,device=DEV,head_idx=min(i,199))
  ...
  rates=[sum(a.user.allocate_band.values()) for a in ua if a.user.sat_connected and a.user.allocate_band]; r=sum(rates)/len(ua) if rates else 0
  print(f"  cy{cy+1} d={d} rate={r:.0f}",flush=True)

torch.save(c.evaluate_net,f"log/model/drqn_C_u200_{TAG}_final.pkl")

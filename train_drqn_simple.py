#!/usr/bin/env python3
"""DRQN 杨论文简化: Cavai+RVT(2N+1) + 纯rate reward"""
...
class TestAgent:
    ...
    def decide(self, obs, N):
        if self.lstm_h is None:
            self.lstm_h = torch.zeros(1, 1, H).to(self.device)
            self.lstm_c = torch.zeros(1, 1, H).to(self.device)
        ...
        qv, self.lstm_h, self.lstm_c = self.qnet(
            torch.as_tensor(sl, dtype=torch.float).to(self.device).unsqueeze(0),
            self.lstm_h, self.lstm_c)
        qv = qv[:, -1, :].squeeze(0)
        # Cavai at [0:N], invisible → mask
        for i in range(N):
            if obs[i] <= 0.0:
                qv[i] = -float('inf')
        # ISL-rank diagnostic
        visible = [(i, obs[N+i]) for i in range(N) if obs[i] > 0.0]  # (idx, rvt)
        if visible:
            chosen = qv.argmax().item()
            sorted_by_rvt = sorted(visible, key=lambda x: -x[1])
            chosen_rank = next(r for r, (i, _) in enumerate(sorted_by_rvt) if i == chosen)
            isl_rank = chosen_rank / max(1, len(sorted_by_rvt) - 1) if len(sorted_by_rvt) > 1 else 0.0
        else:
            isl_rank = 0.5
        return qv.argmax().item(), isl_rank
...
def test_20eps(qnet, label):
    ...
    agents = [TestAgent(u, qnet, device) for u in users_t]
    for ep in range(5):
        for a in agents:
            obs = env_t.Observe(a.user, 'NETWORK_LOAD')
            act, _ = a.decide(obs, N)
    ...
    for ep in range(20):
        ...
        for a in agents:
            ...
            isl_ranks.append(isl_r)
        rates.append(ep_rate/n if n else 0)
        env_t.Update_Env((ep+1)*30, 'NETWORK_LOAD')
    avg = sum(rates)/20; l50 = sum(rates[-10:])/10; isl = sum(isl_ranks)/len(isl_ranks)
    print(f'  [TEST {label}] avg={avg:.0f} last50={l50:.0f} ISLrank={isl:.3f}', flush=True)
    return avg, l50, isl

print('DRQN Simple: Cavai+RVT(2N+1) pure-rate-reward ISL=2000 200u 200eps', flush=True)
env = build_drqn_env(200, 'C'); env.reset(0, 'NETWORK_LOAD')
N = env.topo.total_sat; users = list(env.topo.user)

qnet = Q_net(2*N+1, N, H).to(device)
...
class Agent:
    ...
    def decide(self, obs):
        ...
        qv, self.lstm_h, self.lstm_c = self.qnet(
            ...
            self.lstm_h, self.lstm_c)
        qv = qv[:, -1, :].squeeze(0)
        # Cavai at [0:N], -1 or <=0 means invisible
        for i in range(N):
            if obs[i] <= 0.0:
                qv[i] = -float('inf')
...
for ep in range(200):
    ...
    if ep >= SEQ+1:
        ...
        if len(s_l) > 0:
            ...
            with torch.no_grad():
                ...
                # Cavai at [0:N], <=0 means invisible
    ...
    if ep > 0 and ep % 50 == 0:
        torch.save(qnet.state_dict(), f'./log/model/drqn_simple_ep{ep}.pkl')

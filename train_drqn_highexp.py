#!/usr/bin/env python3
"""DRQN 实验3: 高探索 ε=0.1 + 原始seq + γ=0.9 — 隔离"ε太小"问题"""
...
print('DRQN Exp3: ε=0.1 + seq + γ=0.9 + Cavai-only(N+1)', flush=True)
...
class Agent:
    def __init__(self, u, q):
        self.user = u; self.qnet = q; self.epsilon = EPSILON; self.sequence = SEQ
        self.state_fifo = collections.deque(maxlen=SEQ)
...
for ep in range(200):
    ...
    if ep > 0 and ep % 50 == 0:
        torch.save(qnet.state_dict(), f'./log/model/drqn_highexp_ep{ep}.pkl')

#!/usr/bin/env python3
"""测试 60dB vs 80dB: MGCS vs MaxISL (独立进程, 确保每个功率级别模块重载)"""
import os, random, math, sys, subprocess
from math import pi as _pi

def run_one(db, strategy, epn=20):
    """Run one strategy in a subprocess with given power level"""
    code = f"""
...
os.environ['LEO_PAPER_PT_GT_GS_DB'] = '{db}'
...
topo = Topology(); o, s = 16, 16; ph = 1
fp = 2.0*ph*_pi/(o*s); lean = 54.0/180.0*_pi; theta = 2.0*_pi/o
topo.Add_Constellation(o, s, SAT_HEIGHT, fp, lean, theta, TYPE_2PI)
topo.Each_Satellite()
for lo,la in [(0,0),(60,0),(120,0),(180,0),(-120,0),(-60,0)]:
    topo.Add_Gateway_Loc(lo/180*_pi, la/180*_pi, antenna_Num=5, name_str=f'GW')
random.seed(42); us = [(random.uniform(-120,120)*_pi/180, random.uniform(-60,60)*_pi/180) for _ in range(200)]
...
for ep in range({epn}):
    for user in env.topo.user:
        ...
        for sat in env.ho[user]:
            if '{strategy}' == 'mgcs': v = env.ho[user][sat].c_quality
            else:
                v = 0
    ...
    for u in env.topo.user:
        if u.sat_connected and u.sat_connected in env.ho[u]:
            ...
            if fd and u.sat_connected != fd: isl = env.net.N2N_status[u.sat_connected.con_id-1][u.sat_connected.ID-1][fd.ID-1].free_band
            er += min(env.ho[u][u.sat_connected].c_quality, isl); n += 1
    rates.append(er/n if n else 0)
    tm += 50; env.Update_Env(tm, 'NETWORK_LOAD')
env.close()
avg = sum(rates[5:])/len(rates[5:])
print(f'RESULT: avg={{avg:.1f}} cq_min={{min(cqs):.0f}} cq_max={{max(cqs):.0f}} cq_mean={{sum(cqs)/len(cqs):.0f}}')
"""
    env = os.environ.copy()
    env['LEO_PAPER_PT_GT_GS_DB'] = str(db)
    # Write code to temp file
    with open('/tmp/_test_power.py', 'w') as f:
        f.write(code)
    result = subprocess.run([sys.executable, '-u', '/tmp/_test_power.py'],

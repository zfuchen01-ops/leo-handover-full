#!/usr/bin/env python3
"""DRQN 实验4: MLP(无LSTM seq=1) + γ=0.9 + Cavai-only — 隔离"LSTM过度复杂"问题"""
...
class MLP_QNet(nn.Module):
    """简单MLP: input→256→128→actions, 无LSTM"""
    def __init__(self, input_dim, action_dim, hidden=H):
        ...
        self.net = nn.Sequential(
            ...
            nn.Tanh()  # Q in [-1, 1], matches reward range
        )
    def forward(self, x):
...
def test_20eps(qnet, label):
    ...
    class TA:
        def __init__(self, u, q):
            self.user = u; self.qnet = q; self.device = device
        def decide(self, obs):
            qv = self.qnet(torch.as_tensor(obs, dtype=torch.float).to(device).unsqueeze(0).unsqueeze(0))
            qv = qv.squeeze()
            for i in range(N):
...
print('DRQN Exp4: MLP(no-LSTM) + seq=1 + γ=0.9 + Cavai-only(N+1)', flush=True)
env = build_drqn_env(200, 'C'); env.reset(0, 'NETWORK_LOAD')
N = env.topo.total_sat; users = list(env.topo.user)

qnet = MLP_QNet(N+1, N).to(device)
...
class Agent:
    def __init__(self, u, q):
        self.user = u; self.qnet = q; self.epsilon = EPSILON; self.device = device
    def decide(self, obs):
        if np.random.rand() < self.epsilon:
            return np.random.choice([s.ID-1 for s in self.user.sat_covered]) if self.user.sat_covered else 0
        qv = self.qnet(torch.as_tensor(obs, dtype=torch.float).to(device).unsqueeze(0).unsqueeze(0)).squeeze()
        for i in range(N):
            if obs[i] <= 0.0: qv[i] = -float('inf')
        return qv.argmax().item()
...
for ep in range(200):
    ...
    for a in agents:
        ...
        replay_buf.append((obs, act, r, obs))  # next_state=same (MLP no temporal dependency)
    rates.append(ep_rate/n if n else 0)

    if len(replay_buf) >= BATCH:
        batch = random.sample(replay_buf, BATCH)
        s_b = torch.FloatTensor([b[0] for b in batch]).to(device).unsqueeze(1)  # (B,1,F)
        a_b = torch.LongTensor([b[1] for b in batch]).to(device)
        r_b = torch.FloatTensor([b[2] for b in batch]).to(device)
        ns_b = torch.FloatTensor([b[3] for b in batch]).to(device).unsqueeze(1)  # (B,1,F)
        with torch.no_grad():
            nq = target_net(ns_b)  # (B, N)
            nq[~(ns_b[:, -1, :N] > 0.0)] = -float('inf')
            target = r_b + 0.9 * nq.max(dim=1).values
        q = qnet(s_b).gather(1, a_b.unsqueeze(1)).squeeze(1)  # (B,)
    ...
    if ep > 0 and ep % 50 == 0:
        torch.save(qnet.state_dict(), f'./log/model/mlp_simple_ep{ep}.pkl')
